#!/usr/bin/env bash
# Lists models available to fable-lite external runs: local Ollama models, Ollama cloud models, and current routes.
set -uo pipefail
BASE_URL="${FABLE_LITE_EXTERNAL_BASE_URL:-http://localhost:11434}"
echo "## Endpoint: $BASE_URL"
if curl -s -m 3 "$BASE_URL/api/version" >/dev/null 2>&1; then
  echo "reachable: yes ($(curl -s -m 3 "$BASE_URL/api/version" | tr -d '\n'))"
else
  echo "reachable: NO. Start it with: ollama serve   (or set FABLE_LITE_EXTERNAL_BASE_URL)"
fi
...
echo; echo "## Routes"
for f in "$PWD/.fable-lite/config.json" "$HOME/.claude/fable-lite.json"; do
  [ -f "$f" ] && echo "from $f:" && python3 -c "import json,sys; print(json.dumps(json.load(open(sys.argv[1])).get('external',{}), indent=2))" "$f" && break
done
[ -f "$PWD/.fable-lite/config.json" ] || [ -f "$HOME/.claude/fable-lite.json" ] && true || echo "(no config; all tiers run on Anthropic subagents. Create .fable-lite/config.json to route tiers externally.)"
