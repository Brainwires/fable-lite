#!/usr/bin/env bash
# jev orchestration-run stats.
# Aggregates the per-run JSON in .jev/runs/ so you can see the offload:
# how many runs, turns, and tokens went to each external model.
#
# Usage:
#   external-stats.sh [--dir <runs-dir>] [--since YYYY-MM-DD]
#     --dir    runs directory (default: <cwd>/.jev/runs)
