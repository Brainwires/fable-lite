...
allowed-tools: Read, Grep, Glob, Bash(git *), Bash(ls *), Bash(cat *), Bash(mkdir *), Bash(jev-run *), Bash(jev-models *), Write, Agent
---
...
2. **Score and brief exactly as `/jev:delegate` does.** Read `${CLAUDE_PLUGIN_ROOT}/skills/jev orchestration/references/routing-rubric.md` and `${CLAUDE_PLUGIN_ROOT}/skills/jev orchestration/references/brief-template.md`. External models get the Sonnet-tier treatment regardless of how capable they are: a typed, numbered Steps section is required, the exemplar is named, and no design is left open. If the task is Fable-tier by the rubric, say so and stop. If context is missing, run one scout (external with `--role scout`, or `jev:scout`) with all questions in one list.

3. **Write the brief to a file** under `.jev/briefs/<slug>.md` in the project (create the directory). The runner reads it from there.

4. **Dispatch:**
   ```
   jev-run --model <model> --brief .jev/briefs/<slug>.md [--role scout|verifier]
   ```
   Run it in the background when dispatching more than one, so they proceed in parallel. Each run writes its full JSON to `.jev/runs/`.

5. **Audit on Fable** with `${CLAUDE_PLUGIN_ROOT}/skills/jev orchestration/references/audit-checklist.md`. Read the diff, not the transcript. Treat a redesign or a non-None "Design choices I made" the same as with a Sonnet agent: yank back to typed steps on the same model. After a second miss on the same item, escalate to `jev:opus-implementer` or take over. Do not escalate to a different external model; that only spends more tokens learning the same thing.
